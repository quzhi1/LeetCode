...
public class Leet472 {
  ...
  public static boolean canForm(final String word, final List<String> dict) {
    for (int i = dict.size() - 1; i >= 0; i--) {
      int foundIndex = word.indexOf(dict.get(i));
      if (foundIndex != -1) {
        String left = word.substring(0, foundIndex);
        String right = word.substring(foundIndex + dict.get(i).length(), word.length());
        boolean canLeftForm = left.isEmpty() || canForm(left, dict);
        boolean canRightForm = right.isEmpty() || canForm(right, dict);
        if (canLeftForm && canRightForm) {
          return true;
        }
      }
    }
  ...
  public static void main(String[] args) {
    ...
    List<String> output1 = findAllConcatenatedWordsInADict(input1);
    System.out.println(StringUtils.join(output1, ","));
    String[]
        input2 =
    ...
    List<String> output2 = findAllConcatenatedWordsInADict(input2);
    System.out.println(StringUtils.join(output2, ","));
  }
}
